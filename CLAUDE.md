...
- **`build-mcp`** — the "make it agent-accessible" tier: generates a **read-only** MCP server in a new top-level `mcp/` folder over the built app's data layer (`DataStore`), so AI agents (Claude/ChatGPT) can query it in natural language. Read-only **by construction** (no write tools); Tier-0 local stdio over mock data (no keys); the remote go-live (Streamable HTTP + OAuth + real backend) is a keyed checklist the user owns. Attended, one confirm gate, re-runnable; never in the loop, and **deliberately not in `autopilot`** (like `deploy`/`polish`). Vetted → `outputs/vetting/2026-07-03-agent-accessibility/`. See `docs/BUILD-MCP.md` and the roadmap `docs/PATH-TO-PRODUCTION.md`.
...
- **`maintain-app`** — the "operate & maintain" tier (path-to-production rung 7): a **report-first** scheduled loop for a shipped `app/` — re-runs the quality signals (`npm audit`/`test-app`/`audit-app`) into ONE plain-language health report, **delegates** dependency/CVE patching to Dependabot/Renovate (~7-day cooldown; surfaced, never auto-bumped), and opens a gated **SAFE-PR** only for a narrow reversible fix class (never a dep bump) — the PR is the exception, not the deliverable. **Never merges/deploys/publishes/enters keys** — a sentinel-scoped `PreToolUse` guard hook + branch protection enforce it. Hybrid scaffold (local Routine sandbox + keyed CI); attended setup; **its own schedule — never in `maintenance-loop`, deliberately not in `autopilot`**. See `docs/MAINTAIN-APP.md` and the roadmap `docs/PATH-TO-PRODUCTION.md`.
