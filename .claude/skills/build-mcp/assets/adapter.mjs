// ---------------------------------------------------------------------------
// GENERIC DataStore -> MCP adapter (shipped asset — copied verbatim into a
// generated mcp/ folder by the build-mcp skill). It knows NOTHING about any
// specific app: given a store + entity catalog it auto-registers read-only
// list_<plural> and get_<name> tools. This one file works for every app.
// Everything app-specific lives in the generated catalog.mjs.
//
// READ-ONLY BY CONSTRUCTION: there is no code path here that registers a
// create/update/delete tool. Write tools would be a separate, opt-in adapter
// gated behind confirmation — they are simply absent. An agent using a server
// built on this adapter can read the app's data but can never change it.
//
// Proven in the 48h spike (outputs/vetting/2026-07-03-agent-accessibility/):
// a real Claude agent answered cross-entity questions from these tools alone.
// ---------------------------------------------------------------------------
import { z } from "zod";
