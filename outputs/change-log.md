...
- 2026-07-06 — build-app — "score first" front door (incremental re-run; the algorithm's #1 cut — put the wedge first): new /score route (app/src/pages/Score.tsx) — a 3-field quick score (traction/team/ask) → readiness ring + worst-first gaps → "Build my full deck" graduates into a prefilled editor (nothing saved until then); new coaching.ts analyzeQuick + quickVerdict reuse the existing per-slide rules (analyzeDeck untouched, so the fast + full scores never disagree); Home hero CTA swap ("Score my pitch — free" primary) + nav link. User-approved via brainstorming; typecheck + build clean; browser-verified (strong/thin/strong → 81 green, Team=FIX first, exact graduation). Record: raw/builds/2026-07-06-score-first.md — applied
