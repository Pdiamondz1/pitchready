...
git diff --name-only main..HEAD | grep -qE '^outputs/polish/[0-9]' && echo "POLLUTION output" || echo "OK no dated polish output added"
git diff --name-only main..HEAD | grep -qE '^raw/builds/' && echo "POLLUTION raw/builds (branch added a build record)" || echo "OK no raw/builds record added (pre-existing .gitkeep is fine)"
git diff --name-only main..HEAD | grep -qE '^wiki/build\.md' && echo "POLLUTION wiki/build" || echo "OK no wiki/build.md added"
...
```
Expected: `OK no app/ committed`, `OK no dated polish output added`, `OK no raw/builds record added (pre-existing .gitkeep is fine)`, `OK no wiki/build.md added`, `OK .gitkeep present`, `OK outputs/polish clean`. *(The `raw/builds/`, `outputs/polish/`, and `wiki/build.md` checks scope to the branch diff — `raw/builds/.gitkeep` ships on `main` as template scaffolding, so a whole-tree `git ls-files` check would false-positive; what matters is that this phase adds no real build record.)*
