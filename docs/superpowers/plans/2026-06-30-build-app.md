...
- [ ] **Step 3: `setup-project`** — read the skill, find the "Offer follow-ups" / post-setup offers
  step (the one that lists `cd aios && npm install`, indexing, scheduling, etc.). That step is a
  numbered prose item, not a sub-bulleted list — so add the offer as **a new clause in that step, or a
  short sub-bullet under it**, gated on charter + design existing, e.g.:
...
Keep it in the surrounding voice; do not change any other offer.
...
- [ ] **Step 2: Invariants intact.** The real invariant is: no *existing* `raw/` content mutated, and
  the ONLY `raw/` addition is the shipped `raw/builds/.gitkeep` placeholder (added by Task 1).

```bash
git diff --name-only main..HEAD -- .claude/skills/improve-system outputs/change-log.md   # expect EMPTY
git diff --name-only main..HEAD -- raw ':(exclude)raw/builds/.gitkeep'                     # expect EMPTY
```
Expect both EMPTY (improve-system and the change-log ledger untouched; the only raw/ change is the new
`raw/builds/.gitkeep`). **Do NOT delete `raw/builds/.gitkeep`** — it is an intended, shipped file.
